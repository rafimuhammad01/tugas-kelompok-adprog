...
class TugasControllerTest {
    ...
    @Test
    void testGetListTugas() throws Exception {
        Iterable<Tugas> listTugas = Arrays.asList(tugas);
        when(tugasService.getTugas(tugas.getId())).thenReturn(tugas);
        when(tugasService.getListTugas()).thenReturn(listTugas);

        System.out.println(tugasService.getListTugas());
        mvc.perform(get("/tugas/1").header("Authorization", "Bearer " +  getJWTToken())
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk()).andExpect(content()
                .contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$.data[1].id").value(tugas.getId()));
    }
